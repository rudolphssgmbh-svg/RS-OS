...
function createRuntimeMeasurementWriteService({
  ...
  if (!db || typeof db.query !== 'function') {
    throw new Error('runtime measurement service requires db.query');
  }

  if (typeof writeEvent !== 'function') {
    throw new Error('runtime measurement service requires writeEvent');
  }
  ...
  async function create(input) {
    ...
    /*
     * PF-002C4N / A1:
     * Preserve the existing operation order exactly.
     *
     * No transaction is introduced here.
     * Transaction hardening belongs to a separate authorized step.
     */
